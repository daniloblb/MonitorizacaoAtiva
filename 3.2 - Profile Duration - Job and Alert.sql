...
--- 3.2 - Profile Duration - Job and Alert
***********************************************************************************************************************************/
...
USE MonitorizacaoAtiva -- Database Name
...
BEGIN
	...
	*******************************************************************************************************************************/
	exec @rc = sp_trace_create @TraceID output, 0, N'C:\DATA\Duration', @maxfilesize, NULL 
...
BEGIN
	...
	EndTime, Reads,Writes, CPU, ServerName, DataBaseName, RowCounts/*, SessionLoginName*/
	FROM :: fn_trace_gettable('C:\DATA\Duration.trc', default)
...
BEGIN TRANSACTION
	...
	EXEC @ReturnCode = msdb.dbo.sp_add_jobstep 
			...
			@database_name = N'MonitorizacaoAtiva', 
			@flags = 0
	...
	EXEC @ReturnCode = msdb.dbo.sp_add_jobstep 
			...
			@os_run_priority = 0,
			@subsystem=N'CmdExec',
			@command = N'Del "C:\DATA\Duration.trc" /Q',
			@flags = 0
