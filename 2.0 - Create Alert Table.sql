/***********************************************************************************************************************************
--- MONITORIZAÇÃO ATIVA ABREU VIAGENS
--- 2.0 - Create Alert Table
***********************************************************************************************************************************/
...
VALUES	('Version DB CheckList ',				'2.0',@Fl_Language,									0,1,		NULL,	NULL,			@Ds_Profile_Email,	@Ds_Email,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL),
		...
		SET
				...
				[Ds_Metric_2] = 'minute'
		WHERE [Nm_Alert] IN ('Blocked Process');
		
		UPDATE [dbo].[Alert_Parameter]
		SET
				[Vl_Parameter_2] = 20, --SPID that is generating the lock must be executing for at least 1 minute
				[Ds_Metric_2] = 'minutes'
		WHERE [Nm_Alert] IN ('Blocked Long Process');
...
SET NOCOUNT ON
	...
	IF ( OBJECT_ID('[dbo].Log_Alert_Execution') IS NOT NULL )
		DROP TABLE [dbo].Log_Alert_Execution;
GO
	create table Log_Alert_Execution(
		Id_Log_Alert_Execution int identity,
		Nm_Procedure varchar(100),
		Nm_Alert varchar(100),
		Dt_Execucao datetime)

	create clustered index SK01_Log_Alert_Execution on Log_Alert_Execution(Dt_Execucao)
GO
IF ( OBJECT_ID('[dbo].[stpConfiguration_Table]') IS NOT NULL ) 
	DROP PROCEDURE [dbo].stpConfiguration_Table
GO
...
VALUES	('Version DB CheckList ',				'2.0',NULL,NULL,NULL,@Fl_Language,									0,1,		NULL,	NULL,			@Ds_Profile_Email,	@Ds_Email,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL),
		('Version DB Alert ',					'2.0',NULL,NULL,NULL,@Fl_Language,									0,1,		NULL,	NULL,			@Ds_Profile_Email,	@Ds_Email,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL),
